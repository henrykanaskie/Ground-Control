...
/** The inert placeholder a project that blew up mid-scan still gets. */
function failedProject(id, name, projPath, err) {
  return {
    id,
    name,
    path: projPath,
...
/**
 * The projects one source contributes, as `{ dir, name }` pairs.
 *
 *   kind "root"     every candidate directory directly inside it
 *   kind "project"  the folder itself, scanned from its parent
 *
 * Never throws: a source that has been moved or unmounted contributes nothing.
 */
function projectsInSource(source) {
  const abs = path.resolve(source.path);
  if (source.kind === 'project') {
    try { if (!fs.statSync(abs).isDirectory()) return []; } catch { return []; }
    const name = path.basename(abs);
    if (!name) return [];
    return [{ dir: path.dirname(abs), name }];
  }
  return listProjectDirs(abs).map((name) => ({ dir: abs, name }));
}

/**
 * Scan every source in order and merge the result into one payload.
 *
 * Ids are assigned source by source, alphabetically within each source, so a
 * project keeps the same id no matter what is added after it: bookmarks and
 * Forge staging directories both depend on that.
 *
 * A project reachable through two sources (a nested root, a symlink) appears
 * once, owned by the first source that claimed it.
 *
...
async function scanSources(sources) {
  ...
  for (const source of list) {
    for (const { dir, name } of projectsInSource(source)) {
      const projPath = path.join(dir, name);
      // Dedupe on the real path: the same folder reached two ways is one card.
      let real = projPath;
      try { real = fs.realpathSync(projPath); } catch { /* not there, keep the literal */ }
      if (seenReal.has(real)) continue;
      seenReal.add(real);
      jobs.push({ source, dir, name, id: uniqueSlug(slugify(name), used) });
    }
  }
  ...
  const results = await Promise.all(jobs.map(async (job) => {
    let p;
    try {
      p = await scanProject(job.dir, job.name, job.id, started);
    } catch (err) {
      p = failedProject(job.id, job.name, path.join(job.dir, job.name), err);
    }
    // Where this project came from: the anchor every path guard uses, and the
    // label the grid groups by.
    p.sourceId = job.source.id;
    p.sourceKind = job.source.kind;
    p.sourcePath = path.resolve(job.source.path);
    p.sourceLabel = job.source.label || path.basename(path.resolve(job.source.path)) || job.source.path;
    return p;
  }));
  ...
  const firstRoot = list.find((s) => s.kind === 'root') || list[0] || null;

  return {
    root: firstRoot ? path.resolve(firstRoot.path) : null,
...
/**
 * Scan every project under a single `root`.
 * The one-folder case of `scanSources`, kept because most callers still think
 * in terms of one root.
 */
async function scanRoot(root) {
  const rootAbs = path.resolve(root);
  return scanSources([{ id: 'root', kind: 'root', path: rootAbs, label: null }]);
}
...
export {
  scanRoot, scanSources, scanProject, projectDetail, buildTree, listProjectDirs,
  projectsInSource, walkProject,
