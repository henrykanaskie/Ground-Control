...
const BLOCKER_LABEL = {
  ...
  'not-a-direct-child': 'Not a plain folder in the folder it was scanned from',
};
...
function checkDirectChild(root, project) {
  ...
  try {
    ...
    catch { out.reason = 'the path does not resolve inside the folder it was scanned from'; return out; }
    if (path.resolve(projPath) !== abs) { out.reason = 'the project path is not <root>/<name>'; return out; }
    if (abs === rootResolved) { out.reason = 'that is the watched folder itself'; return out; }
    if (path.dirname(abs) !== rootResolved) { out.reason = 'it is nested, not a direct child of the folder it was scanned from'; return out; }
    ...
    if (path.dirname(realTarget) !== realRoot) {
      out.reason = 'it resolves outside the folder it was scanned from';
      return out;
    }
 ...
 * Assess one project.
 *
 * `opts.root`: the folder this project was scanned from, which is what the
 *                direct-child gate is measured against. For a folder added on
 *                its own that is its parent directory (CONTRACT-SOURCES.md §3).
...
async function assessProject(project, opts = {}) {
  ...
  if (!gate.ok) {
    blockers.push(blocker('not-a-direct-child',
      `Reclaim only ever touches a plain folder sitting directly in a watched folder: ${gate.reason}.`));
  }
  ...
  /* CONTRACT-COMPLETE.md: a finished project and an abandoned one look
   * identical from here. The user has already told Ground Control which this
   * is, and offering to bin something they just declared done would make the
   * mark worthless. This is a blocker rather than a score penalty because it
   * is an answer, not evidence: no amount of emptiness outweighs it. */
  if (project && project.completed) {
    blockers.push(blocker('marked-complete',
      'This project is marked complete: finished on purpose, not abandoned.'));
  }
...
/**
 * Assess every project in one pass: one agent sweep for the whole list.
 * Never rejects: a project that blows up comes back blocked and inert.
 *
 * `opts.rootFor(project)` supplies the folder each project was scanned from,
 * which is what the direct-child gate is measured against. Projects can now
 * come from several different folders, so one global root is not enough.
 */
async function assessAll(projects, opts = {}) {
  const list = Array.isArray(projects) ? projects : [];
  const now = Date.now();
  const rootFor = typeof opts.rootFor === 'function' ? opts.rootFor : () => opts.root;
 ...
 * `opts.root`        is the folder it was scanned from; it must be a direct child.
...
async function trashProject(project, opts = {}) {
  ...
  if (!root) throw new ReclaimError(500, 'the folder this project was scanned from is unknown, so nothing can be removed');
