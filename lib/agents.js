...
/**
 * Every transcript store a project could be writing to: its own path, plus the
 * cwd of each live process attributed to it.
 *
 * Claude Code keys the store on the session's cwd, so these differ whenever an
 * agent works in a subfolder, or the project folder was renamed after the
 * session started. Resolving only the project root made those agents invisible.
 *
 * Exported for testing; `resolve` is injected so the test does not depend on
 * what happens to be in ~/.claude/projects.
 */
function transcriptDirsFor(projectPath, procs, resolve = transcriptDirFor) {
  const dirs = new Set();
  let root = null;
  try { root = resolve(projectPath); } catch { root = null; }
  if (root) dirs.add(root);
  for (const proc of procs || []) {
    if (!proc || !proc.cwd) continue;
    let d = null;
    try { d = resolve(proc.cwd); } catch { d = null; }
    if (d) dirs.add(d);
  }
  return { root, dirs: [...dirs] };
}
...
/**
 * The sessions worth a tail read when deciding how many agents are mid-turn.
 *
 * Two agents started in the SAME directory write two transcripts into the same
 * store, so reading only the newest session per directory could never see more
 * than one of them: a project with three agents working in it drew a single
 * orbit. Take as many recent sessions per directory as there are live
 * processes, so the count can reach the number of agents actually running.
 *
 * Sessions silent past STALL_WINDOW_MS are dropped here rather than read: they
 * cannot be mid-turn (the caller rejects them anyway), and dropping them early
 * spends the read budget on sessions that could still be working. Newest
 * first, capped at TURN_READS_MAX so a project with a long history pays a
 * handful of cached reads and not one per session it has ever had.
 *
 * `sessions` is injected so the test does not depend on what happens to be in
 * ~/.claude/projects.
 */
function turnCandidates(dirs, live, nowMs, sessions = sessionsIn) {
  const perDir = Math.max(1, Math.min(live | 0, TURN_READS_MAX));
  const out = [];
  for (const d of dirs || []) {
    let found = [];
    try { found = sessions(d) || []; } catch { found = []; }
    for (const sess of found.slice(0, perDir)) {
      if (nowMs - sess.mtimeMs > STALL_WINDOW_MS) continue;
      out.push(sess);
    }
  }
  out.sort((x, y) => y.mtimeMs - x.mtimeMs);
  return out.slice(0, TURN_READS_MAX);
}
...
async function agentActivity(projects, opts = {}) {
  ...
  try {
    ...
    for (const p of list) {
      ...
      // Every transcript store this project could be writing to — the project
      // root AND the cwd of each live process.
      //
      // Resolving only the project root was a real bug: Claude Code keys its
      // transcript directory on the session's cwd, so an agent working in a
      // SUBFOLDER (`ideas/team_dispatch`) writes to `-…-ideas-team-dispatch`,
      // not `-…-ideas`. The project's own store then looks untouched, step 3
      // parks it, `live` drops to 0, and step 4 — which does know about
      // process cwds — never runs, because it only visits live projects. An
      // agent actively working in a subfolder was reported as parked.
      //
      // The same failure hits a renamed folder: the running session keeps the
      // store it was created with, under the old name.
      const resolved = transcriptDirsFor(p.path, a._procs);
      const dirs = new Set(resolved.dirs);
      a._dirs = resolved.dirs;
      a._dir = resolved.root || resolved.dirs[0] || null;
      ...
      for (const d of dirs) {
        let sessions = [];
        try { sessions = sessionsIn(d); } catch { sessions = []; }
        total += sessions.length;
        if (sessions[0] && (!newest || sessions[0].mtimeMs > newest.mtimeMs)) newest = sessions[0];
      }
      a.sessionCount = total;
      if (newest) {
        a.lastSessionISO = toISO(newest.mtimeMs);
    ...
    for (const p of liveProjects) {
      ...
      try {
        ...
        // Same set step 2 resolved: project root plus every live process cwd.
        const dirs = new Set(a._dirs || []);
        if (a._dir) dirs.add(a._dir);

        const nowMs = Date.now();
        const candidates = turnCandidates([...dirs], a.live, nowMs);
        if (!candidates.length) continue;

        let workingCount = 0;
        let actionFrom = null;

        // Already capped and stall-filtered by turnCandidates; the stall check
        // is repeated because it is the rule that matters: a mid-turn session
        // silent too long means the process died holding the pen, and a stale
        // `tool_use` must not spin an orbit forever.
  ...
  for (const a of map.values()) { delete a._procs; delete a._dir; delete a._dirs; }
